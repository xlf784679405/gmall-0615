...
public class SearchService {
    ...
    public SearchResponse search(SearchParamVO searchParamVO) {
        ...
        try {
            String sdl = buildDSL(searchParamVO);
            System.out.println(sdl);
            Search search = new Search.Builder(sdl).addIndex("goods").addType("info").build();
    ...
    private String buildDSL(SearchParamVO searchParamVO) {
        ...
        if (ArrayUtils.isNotEmpty(catelog3s)){
            boolQuery.filter(QueryBuilders.termQuery("productCategoryId" , catelog3s));
        }
        //搜索的规格属性的过滤
